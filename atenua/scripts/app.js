...
function calcularProteccionSupuesta() {
    let atenuacionMediaInputs = Array.from(
        document.querySelectorAll("#filaAtenuacion input"),
    );
    let desviacionStandardInputs = Array.from(
        document.querySelectorAll("#filaDesviacion input"),
    );
    let proteccionSupuestaInputs = Array.from(
        document.querySelectorAll("#filaProteccion input"),
    );
    ...
    if (!datosCompletos) {
        alert(
            "Por favor, llene todos los datos. La columna de la frecuencia 63 Hz es optativa.",
        );
        return;
    }
    ...
    for (let i = 0; i < atenuacionMediaInputs.length; i++) {
        ...
        if (valorAtenuacion === "" || valorDesviacion === "") {
            ...
            proteccionSupuestaInputs[i].value = (
                parseFloat(valorAtenuacion) - parseFloat(valorDesviacion)
            ).toFixed(1);
        }
    }

    let espectroMedidoInputs = Array.from(
        document.querySelectorAll("#filaEspectroMedido input"),
    );
    let espectroProtegidoInputs = Array.from(
        document.querySelectorAll("#filaEspectroProtegido input"),
    );
    ...
    for (let i = 0; i < espectroMedidoInputs.length; i++) {
        ...
        if (valorMedido === "" || valorSupuesta === "") {
            ...
            espectroProtegidoInputs[i].value = (
                parseFloat(valorMedido) - parseFloat(valorSupuesta)
            ).toFixed(1);
        }
    }
...
function calcularEspectro(filaId) {
    ...
    let lA = [];
    let espectroInputs = Array.from(
        document.querySelectorAll(`#${filaId} input`),
    );
    ...
    for (let i = 0; i < espectroInputs.length; i++) {
        ...
        if (valorMedido === "" || isNaN(parseFloat(valorMedido))) {
            console.error(
                `Valor no numérico o vacío en #${filaId}:`,
                valorMedido,
            );
            continue;
        }
...
function calcularTodo() {
    ...
    document.querySelector("#filaRuidoMedido .leq-resultado input").value =
        leqMedido.toFixed(1);
    document.querySelector("#filaRuidoAtenuado .leq-resultado input").value =
        leqAtenuado.toFixed(1);

    const elIndice = document.getElementById("indice-proteccionBo");
...
function borrarTodoBo() {
    ...
    const elIndiceBo = document.getElementById("indice-proteccionBo");
    elIndiceBo.textContent = "";
    elIndiceBo.classList.remove(
        "indice-sobreprotegido",
        "indice-buena",
        "indice-aceptable",
        "indice-insuficiente",
...
function guardarEnSessionStorageBandasOctava() {
    ...
    sessionStorage.setItem(
        "bo_nombreProtector",
        document.getElementById("nombre-protector").value,
    );
    sessionStorage.setItem(
        "bo_lugarRuido",
        document.getElementById("lugar-ruido").value,
    );
    sessionStorage.setItem(
        "bo_cbox2",
...
function guardarValoresTabla(filaId, prefix = "") {
    // Parámetro prefix
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document
        .getElementById("nombre-protector")
        .addEventListener("input", saveAllData);
    document
        .getElementById("lugar-ruido")
        .addEventListener("input", saveAllData);
...
function cargarDatosDesdeSessionStorage() {
    ...
    document.getElementById("nombre-protector").value =
        sessionStorage.getItem("bo_nombreProtector") || "";
    document.getElementById("lugar-ruido").value =
        sessionStorage.getItem("bo_lugarRuido") || "";
    document.querySelector("#filaRuidoMedido .leq-resultado input").value =
        sessionStorage.getItem("bo_leqMedido") || "";
    document.querySelector("#filaRuidoAtenuado .leq-resultado input").value =
        sessionStorage.getItem("bo_leqAtenuado") || "";
    document.getElementById("cbox2").checked =
        sessionStorage.getItem("bo_cbox2") === "true";
    const elIndiceBoRestore = document.getElementById("indice-proteccionBo");
    elIndiceBoRestore.textContent =
        sessionStorage.getItem("bo_indiceProteccion") || "";
    const storedClase = sessionStorage.getItem("bo_indiceProteccionClase");
    if (storedClase) {
        elIndiceBoRestore.className = storedClase;
    }
...
function cargarValoresTabla(filaId, prefix = "") {
    // Parámetro prefix
